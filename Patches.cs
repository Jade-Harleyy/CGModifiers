...
namespace CGModifiers
{
    ...
    internal static class Patches
    {
        ...
        private static void EndlessGrid_OnTriggerEnter_Prefix(Collider other, EndlessGrid __instance, ref int ___maxPoints)
        {
            ...
            ___maxPoints = Mathf.RoundToInt(ValueManager.Instance.startingPointsSelector.Value * (ValueManager.Instance.forceRadianceToggler.Value ? 3 : 1));
            for (int i = 1; i <= __instance.startWave - 1; i++)
            {
                ___maxPoints += Mathf.RoundToInt((3 + i / 3) * (ValueManager.Instance.pointMultiplierSelector.Value * (ValueManager.Instance.forceRadianceToggler.Value ? 3 : 1) - 1));
            }

            Transform zapZone = GameObject.Find("/Everything").transform.GetChild(4);
            zapZone.localPosition = new(zapZone.localPosition.x, ValueManager.Instance.zapZoneHeightSelector.Value * 2.5f + 10.5f, zapZone.localPosition.z);
        ...
        private static void EndlessGrid_OnTriggerEnter_Postfix(Collider other)
        {
            ...
            GameObject.Find("/Everything").transform.GetChild(4).gameObject.SetActive(ValueManager.Instance.scoreboardToggler.Value);
        }
        ...
        {
            ___maxPoints += Mathf.RoundToInt((3 + (__instance.currentWave + 1) / 3) * (ValueManager.Instance.pointMultiplierSelector.Value * (ValueManager.Instance.forceRadianceToggler.Value ? 3 : 1) - 1));
        }
        ...
        {
            if (ValueManager.Instance.forceRadianceToggler.Value)
            {
                __result = true;
                return false;
            }
